...
public class FileUtils {
    ...
    public static String copyAssetsFile2Phone(Context context, String fileName) {
        ...
        try {
            ...
            Log.d("MovieMediaPlayer",fileName+"---fileName---0");
            // 如果文件不存在（新建），或者文件大小不一致（覆盖）
            if (!file.exists() || file.length() != inputStream.available()) {
                Log.d("MovieMediaPlayer",fileName+"---fileName---1");
            ...
            Log.d("MovieMediaPlayer",fileName+"---fileName---2");
            e.printStackTrace();
        } finally {
            Log.d("MovieMediaPlayer",fileName+"---fileName---3");
            if (inputStream != null) {
                try {
                    inputStream.close();
